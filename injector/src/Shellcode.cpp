...
bool Shellcode::Map() {
  ...
  HANDLE file = CreateFileW(filepath, GENERIC_READ, FILE_SHARE_READ, NULL,
 ...
 */
bool Shellcode::InjectInProcess(Process* process,
  ...
  auto* nt_headers =
      reinterpret_cast<IMAGE_NT_HEADERS*>(base + dos_header->e_lfanew);
  auto* section_table = reinterpret_cast<IMAGE_SECTION_HEADER*>(
      base + dos_header->e_lfanew + sizeof(IMAGE_NT_HEADERS));
  ...
  DWORD new_imagebase = process->VirtualAlloc(
      0, nt_headers->OptionalHeader.SizeOfImage, MEM_RESERVE, PAGE_NOACCESS);
  if (!new_imagebase) {
  ...
  for (size_t i = 0; i < nt_headers->FileHeader.NumberOfSections; i++) {
    ...
    debug_printf("  Allocating the section '%s' at 0x%08X of size 0x%08x\n",
                 section_table[i].Name, target_addr, allocation_size);
    DWORD success = process->VirtualAlloc(target_addr, allocation_size,
    ...
    // Copy the data
    void* src =
        reinterpret_cast<void*>(base + section_table[i].PointerToRawData);
  ...
  char* relocs =
      reinterpret_cast<char*>(base + reloc_section->PointerToRawData);
  ...
  while (relocs < relocs_end) {
    ...
    auto* relocs_array =
        reinterpret_cast<const WORD*>(relocs + 2 * sizeof(DWORD));
    DWORD nb_relocs = (reloc_block_size - 2 * sizeof(DWORD)) / sizeof(WORD);

    debug_printf(
        "  %d relocs (block_size = 0x%x) to proceed starting at 0x08%x\n",
        nb_relocs, reloc_block_size, reloc_page_rva);
    for (size_t i = 0; i < nb_relocs; i++) {
      ...
      DWORD* local_addr = reinterpret_cast<DWORD*>(
          base + Rva2FileOffset(reloc_page_rva) + offset);
      DWORD new_value =
          *local_addr - nt_headers->OptionalHeader.ImageBase + new_imagebase;

      debug_printf("    *0x%08X = 0x%08X => 0x%08X\n", process_addr,
                   *local_addr, new_value);
  ...
  DWORD hk_end_scene_addr =
      new_imagebase + GetFunctionRVA("_Z10HkEndSceneP16IDirect3DDevice9@4");
  DWORD hk_reset_addr =
      new_imagebase +
      GetFunctionRVA(
          "_Z7HkResetP16IDirect3DDevice9P23_D3DPRESENT_PARAMETERS_@8");
  ...
  *new_reset = hk_reset_addr;
  ...
  process->WriteProcMem(extern_config_addr, config_blob.data(),
                        config_blob.size());
...
DWORD Shellcode::Rva2FileOffset(DWORD rva) {
  ...
  for (size_t i = 0; i < nt_headers->FileHeader.NumberOfSections; i++) {
    if (ADDR_IS_BTW(
            rva, section_table[i].VirtualAddress,
...
DWORD Shellcode::GetFunctionRVA(const char* function_name) {
  ...
  auto* export_dir = reinterpret_cast<IMAGE_EXPORT_DIRECTORY*>(
      base + Rva2FileOffset(data_directory->VirtualAddress));

  // Search the function name
  auto* names = reinterpret_cast<DWORD*>(
      base + Rva2FileOffset(export_dir->AddressOfNames));
  auto* names_ordinal = reinterpret_cast<WORD*>(
      base + Rva2FileOffset(export_dir->AddressOfNameOrdinals));
  auto* functions = reinterpret_cast<DWORD*>(
      base + Rva2FileOffset(export_dir->AddressOfFunctions));
