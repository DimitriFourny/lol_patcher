...
// Windows)
// IMGUI_API is used for core imgui functions, IMGUI_IMPL_API is used for the
// default bindings files (imgui_impl_xxx.h)
...
// allow using as flags and to not pollute the top of this file)
// Use your programming IDE "Go to definition" facility on the names in the
// central column below to find the actual flags/enum lists.
#ifndef ImTextureID
...
typedef unsigned short ImWchar;  // A single U16 character for keyboard
                                 // input/display. We encode them as multi bytes
...
// Main
IMGUI_API ImGuiIO& GetIO();  // access the IO structure (mouse/keyboard/gamepad
                             // inputs, time, various configuration
                             // options/flags)
...
IMGUI_API void NewFrame();  // start a new Dear ImGui frame, you can submit any
                            ...
                            // Render()/EndFrame().
IMGUI_API void EndFrame();  // ends the Dear ImGui frame. automatically called
                            // by Render(), you likely don't need to call that
                            // yourself directly. If you don't need to render
                            // data (skipping rendering) you may call EndFrame()
                            // but you'll have wasted CPU already! If you don't
                            // need to render, better to not create any imgui
                            // windows and not call NewFrame() at all!
IMGUI_API void Render();  // ends the Dear ImGui frame, finalize the draw data.
                          // You can get call GetDrawData() to obtain it and run
                          // your rendering function. (Obsolete: this used to
                          // call io.RenderDrawListsFn(). Nowadays, we allow and
...
IMGUI_API void ShowDemoWindow(bool* p_open = NULL);  // create Demo window
                                                     // (previously called
                                                     // ShowTestWindow).
                                                     ...
                                                     // available in your
                                                     // application!
IMGUI_API void ShowAboutWindow(bool* p_open = NULL);    // create About window.
                                                        // display Dear ImGui
                                                        // version, credits and
                                                        // build/system
                                                        // information.
IMGUI_API void ShowMetricsWindow(bool* p_open = NULL);  // create Metrics/Debug
                                                        // window. display Dear
                                                        // ImGui internals: draw
                                                        // commands (with
...
IMGUI_API bool ShowStyleSelector(const char* label);     // add style selector
                                                      ...
                                                      // listing the default
                                                      // styles.
IMGUI_API void ShowFontSelector(const char* label);  // add font selector block
                                                     // (not a window),
                                                     // essentially a combo
                                                     // listing the loaded
                                                     // fonts.
...
IMGUI_API const char* GetVersion();  // get the compiled version string e.g.
                                     // "1.23" (essentially the compiled value
                                     // for IMGUI_VERSION)
...
IMGUI_API bool IsWindowHovered(
    ImGuiHoveredFlags flags = 0);  // is current window hovered (and typically:
                                   // not blocked by a popup/modal)? see flags
                                   // for options. NB: If you are trying to
                                   // check whether your mouse should be
                                   // dispatched to imgui or to your app, you
                                   // should use the 'io.WantCaptureMouse'
                                   // boolean for that! Please read the FAQ!
IMGUI_API ImDrawList* GetWindowDrawList();  // get draw list associated to the
                                            // current window, to append your
...
IMGUI_API void SetNextWindowSizeConstraints(
    ...
    void* custom_callback_data = NULL);  // set next window size limits. use
                                         ...
                                         // the current size. Sizes will be
                                         // rounded down. Use callback to apply
                                         // non-trivial programmatic
                                         // constraints.
...
IMGUI_API void SetWindowFocus();  // (not recommended) set current window to be
                                  // focused / top-most. prefer using
                                  // SetNextWindowFocus().
IMGUI_API void SetWindowFontScale(float scale);  // set font scale. Adjust
                                                 // IO.FontGlobalScale if you
                                                 // want to scale all windows.
                                                 // This is an old API! For
                                                 // correct scaling, prefer to
                                                 // reload font + rebuild
                                                 // ImFontAtlas + call
                                                 // style.ScaleAllSizes().
...
IMGUI_API void SetScrollHereX(
    float center_x_ratio = 0.5f);  // adjust scrolling amount to make current
                                   // cursor position visible.
                                   // center_x_ratio=0.0: left, 0.5: center,
                                   // 1.0: right. When using to make a
...
IMGUI_API void SetScrollFromPosX(
    ...
    float center_x_ratio = 0.5f);  // adjust scrolling amount to make given
                                   ...
                                   // GetCursorStartPos() + offset to compute a
                                   // valid position.
...
IMGUI_API void PushItemWidth(float item_width);  // set width of items for
                                                 // common large "item+label"
                                                 // widgets. >0.0f: width in
                                                 // pixels, <0.0f align xx
                                                 // pixels to the right of
                                                 // window (so -1.0f always
                                                 // align width to the right
                                                 // side). 0.0f = default to
                                                 // ~2/3 of windows width,
IMGUI_API void PopItemWidth();
IMGUI_API void SetNextItemWidth(float item_width);  // set width of the _next_
                                                    // common large "item+label"
                                                    // widget. >0.0f: width in
                                                    // pixels, <0.0f align xx
                                                    // pixels to the right of
                                                    // window (so -1.0f always
                                                    // align width to the right
...
IMGUI_API float CalcItemWidth();  // width of item given pushed settings and
                                  // current cursor position. NOT necessarily
                                  // the width of last item unlike most 'Item'
                                  // functions.
IMGUI_API void PushTextWrapPos(
    float wrap_local_pos_x = 0.0f);  // word-wrapping for Text*() commands. <
                                     // 0.0f: no wrapping; 0.0f: wrap to end of
                                     // window (or column); > 0.0f: wrap at
                                     // 'wrap_pos_x' position in window local
                                     // space
...
IMGUI_API void SameLine(float offset_from_start_x = 0.0f,
                        float spacing = -1.0f);  // call between widgets or
                                                 // groups to layout them
                                                 // horizontally. X position
...
IMGUI_API void Dummy(const ImVec2& size);  // add a dummy item of given size.
                                           // unlike InvisibleButton(), Dummy()
                                           // won't take the mouse click or be
                                           // navigable into.
...
IMGUI_API void EndGroup();  // unlock horizontal starting position + capture the
                            // whole group bounding box into one "item" (so you
                            // can use IsItemHovered() or layout primitives such
...
IMGUI_API float GetCursorPosX();  //   (some functions are using window-relative
                                  //   coordinates, such as: GetCursorPos,
                                  //   GetCursorStartPos, GetContentRegionMax,
                                  //   GetWindowContentRegion* etc.
IMGUI_API float GetCursorPosY();  //    other functions such as
                                  //    GetCursorScreenPos or everything in
                                  //    ImDrawList::
IMGUI_API void SetCursorPos(const ImVec2& local_pos);  //    are using the main,
                                                       //    absolute coordinate
                                                       //    system.
...
IMGUI_API void AlignTextToFramePadding();  // vertically align upcoming text
                                           // baseline to FramePadding.y so that
                                           // it will align properly to
                                           // regularly framed items (call if
                                           // you have text on a line before a
                                           // framed item)
...
IMGUI_API bool InvisibleButton(const char* str_id,
                               const ImVec2& size);  // button behavior without
                                                     // the visuals, frequently
                                                     // useful to build custom
                                                     // behaviors using the
                                                     // public api (along with
                                                     // IsItemActive,
                                                     // IsItemHovered, etc.)
...
IMGUI_API bool RadioButton(const char* label,
                           bool active);  // use with e.g. if
                                          // (RadioButton("one", my_value==1)) {
                                          // my_value = 1; }
IMGUI_API bool RadioButton(const char* label,
                           int* v,
...
IMGUI_API bool SliderFloat(const char* label,
                           ...
                           float v_max,
                           const char* format = "%.3f",
                           float power = 1.0f);  // adjust format to decorate
                                                 // the value with a prefix or a
                                                 // suffix for in-slider labels
                                                 // or unit display. Use
                                                 // power!=1.0 for power curve
                                                 // sliders
...
IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...)
    IM_FMTARGS(2);  // helper variation to easily decorelate the id from the
                    ...
                    // ID. to align arbitrary text at the same level as a
...
IMGUI_API void TreePush(const char* str_id);  // ~ Indent()+PushId(). Already
                                              ...
                                              // call TreePush/TreePop yourself
                                              // if desired.
...
IMGUI_API bool ListBoxHeader(
    ...
    const ImVec2& size = ImVec2(0, 0));  // use if you want to reimplement
                                         ...
                                         // interactions. if the function return
                                         // true, you can output elements then
                                         // call ListBoxFooter() afterwards.
IMGUI_API bool ListBoxHeader(const char* label,
                             int items_count,
                             int height_in_items = -1);  // "
IMGUI_API void ListBoxFooter();  // terminate the scrolling region. only call
                                 // ListBoxFooter() if ListBoxHeader() returned
                                 // true!
...
IMGUI_API void SetTooltip(const char* fmt, ...)
    IM_FMTARGS(1);  // set a text-only tooltip, typically use with
                    // ImGui::IsItemHovered(). override any previous call to
                    // SetTooltip().
...
// - We default to use the right mouse (ImGuiMouseButton_Right=1) for the Popup
// Context functions.
// (*) You can use IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup) to
// bypass it and detect hovering even when normally blocked by a popup.
...
IMGUI_API bool BeginPopupContextItem(
    ...
    ImGuiMouseButton mouse_button = 1);  // helper to open and begin popup when
                                         // clicked on last item. if you can
                                         // pass a NULL str_id only if the
                                         // previous item had an id. If you want
                                         // to use that on a non-interactive
                                         // item such as Text() you need to pass
                                         // in an explicit ID here. read
                                         // comments in .cpp!
...
IMGUI_API bool BeginPopupContextVoid(
    const char* str_id = NULL,
    ImGuiMouseButton mouse_button = 1);  // helper to open and begin popup when
                                         // clicked in void (where there are no
                                         // imgui windows).
...
IMGUI_API bool OpenPopupOnItemClick(
    const char* str_id = NULL,
    ImGuiMouseButton mouse_button = 1);  // helper to open popup when clicked on
                                         // last item (note: actually triggers
                                         // on the mouse _released_ event to be
                                         // consistent with popup behaviors).
                                         // return true when just opened.
IMGUI_API bool IsPopupOpen(const char* str_id);  // return true if the popup is
                                                 // open at the current begin-ed
...
// - Currently working on new 'Tables' api which will replace columns (see
// GitHub #2957)
...
IMGUI_API void SetTabItemClosed(
    const char* tab_or_docked_window_label);  // notify TabBar or Docking system
                                              // of a closed tab/window ahead
                                              // (useful to reduce visual
                                              // flicker on reorderable tab
                                              // bars). For tab-bar: call after
                                              // BeginTabBar() and before Tab
                                              // submissions. Otherwise call
                                              // with a window name.
...
IMGUI_API bool BeginDragDropTarget();  // call after submitting an item that may
                                       // receive a payload. If this returns
                                       // true, you can call
                                       // AcceptDragDropPayload() +
                                       // EndDragDropTarget()
...
IMGUI_API bool IsItemActive();  // is the last item active? (e.g. button being
                                // held, text field being edited. This will
                                // continuously return true while holding mouse
                                // button on an item. Items that don't interact
                                // will always return false)
IMGUI_API bool
IsItemFocused();  // is the last item focused for keyboard/gamepad navigation?
IMGUI_API bool IsItemClicked(
    ImGuiMouseButton mouse_button = 0);  // is the last item clicked? (e.g.
                                         // button/node just clicked on) ==
                                         // IsMouseClicked(mouse_button) &&
                                         // IsItemHovered()
...
IMGUI_API bool IsItemEdited();  // did the last item modify its underlying value
                                // this frame? or was pressed? This is generally
                                // the same as the "bool" return value of many
                                // widgets.
...
IMGUI_API bool IsItemDeactivated();  // was the last item just made inactive
                                     // (item was previously active). Useful for
...
IMGUI_API bool IsItemDeactivatedAfterEdit();  // was the last item just made
                                              // inactive and made a value
                                              // change when it was active?
                                              // (e.g. Slider/Drag moved).
                                              // Useful for Undo/Redo patterns
                                              // with widgets that requires
                                              // continuous editing. Note that
                                              // you may get false positives
                                              // (some widgets such as
                                              // Combo()/ListBox()/Selectable()
                                              // will return true even when
...
IMGUI_API ImDrawList* GetBackgroundDrawList();  // this draw list will be the
                                                // first rendering one. Useful
                                                // to quickly draw shapes/text
                                                // behind dear imgui contents.
IMGUI_API ImDrawList* GetForegroundDrawList();  // this draw list will be the
                                                // last rendered one. Useful to
                                                // quickly draw shapes/text over
                                                // dear imgui contents.
IMGUI_API ImDrawListSharedData* GetDrawListSharedData();  // you may use this
                                                          // when creating your
                                                          // own ImDrawList
                                                          // instances.
IMGUI_API const char* GetStyleColorName(ImGuiCol idx);    // get a string
                                                        // corresponding to the
                                                        // enum value (for
                                                        // display, saving,
                                                        // etc.).
IMGUI_API void SetStateStorage(ImGuiStorage* storage);  // replace current
                                                        // window storage with
                                                        // our own (if you want
                                                        // to manipulate it
                                                        // yourself, typically
...
IMGUI_API int GetKeyPressedAmount(int key_index,
                                  float repeat_delay,
...
IMGUI_API void CaptureKeyboardFromApp(
    bool want_capture_keyboard_value = true);  // attention: misleading name!
                                               // manually override
                                               // io.WantCaptureKeyboard flag
                                               // next frame (said flag is
                                               // entirely left for your
                                               // application to handle). e.g.
                                               // force capture keyboard when
                                               // your widget is being hovered.
                                               // This is equivalent to setting
                                               // "io.WantCaptureKeyboard =
                                               // want_capture_keyboard_value";
                                               // after the next NewFrame()
                                               // call.
...
IMGUI_API bool IsMouseReleased(ImGuiMouseButton button);  // did mouse button
                                                          // released? (went
                                                          // from Down to !Down)
IMGUI_API bool IsMouseDoubleClicked(
    ImGuiMouseButton button);  // did mouse button double-clicked? a
                               // double-click returns false in
                               // IsMouseClicked(). uses
...
IMGUI_API bool IsMouseHoveringRect(const ImVec2& r_min,
                                   ...
                                   bool clip = true);  // is mouse hovering
                                                       ...
                                                       // (in screen space).
                                                       // clipped by current
                                                       // clipping settings, but
                                                       // disregarding of other
                                                       // consideration of
                                                       // focus/window
                                                       // ordering/popup-block.
IMGUI_API bool IsMousePosValid(
    const ImVec2* mouse_pos = NULL);  // by convention we use
                                      // (-FLT_MAX,-FLT_MAX) to denote that
...
IMGUI_API ImVec2 GetMousePosOnOpeningCurrentPopup();  // retrieve mouse position
                                                      // at the time of opening
                                                      // popup we have
                                                      // BeginPopup() into
                                                      // (helper to avoid user
                                                      // backing that value
                                                      // themselves)
...
IMGUI_API ImVec2 GetMouseDragDelta(
    ...
    float lock_threshold = -1.0f);  // return the delta from the initial
                                    // clicking position while the mouse button
                                    // is pressed or was just released. This is
                                    // locked and return 0.0f until the mouse
                                    // moves past a distance threshold at least
                                    // once (if lock_threshold < -1.0f, uses
                                    // io.MouseDraggingThreshold)
...
IMGUI_API void CaptureMouseFromApp(
    bool want_capture_mouse_value = true);  // attention: misleading name!
                                            // manually override
                                            // io.WantCaptureMouse flag next
                                            // frame (said flag is entirely left
                                            // for your application to handle).
                                            // This is equivalent to setting
                                            // "io.WantCaptureMouse =
                                            // want_capture_mouse_value;" after
...
IMGUI_API void LoadIniSettingsFromDisk(
    const char* ini_filename);  // call after CreateContext() and before the
                                // first call to NewFrame(). NewFrame()
                                // automatically calls
                                // LoadIniSettingsFromDisk(io.IniFilename).
IMGUI_API void LoadIniSettingsFromMemory(
    const char* ini_data,
    size_t ini_size = 0);  // call after CreateContext() and before the first
                           // call to NewFrame() to provide .ini data from your
                           // own data source.
IMGUI_API void SaveIniSettingsToDisk(
    const char* ini_filename);  // this is automatically called (if
                                // io.IniFilename is not empty) a few seconds
                                // after any modification that should be
                                // reflected in the .ini file (and also by
                                // DestroyContext).
IMGUI_API const char* SaveIniSettingsToMemory(
    size_t* out_ini_size = NULL);  // return a zero-terminated string with the
                                   // .ini data which you can save by your own
                                   // mean. call when io.WantSaveIniSettings is
                                   // set, then save data by your own mean and
...
enum ImGuiWindowFlags_ {
  ...
  // [Internal]
  ImGuiWindowFlags_NavFlattened = 1 << 23,  // [BETA] Allow gamepad/keyboard
  ...
  // style.FrameBorderSize=1.0f or style.WindowBorderSize=1.0f to enable borders
  // around items or windows.
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_CallbackAlways = 1 << 8,  // Callback on each iteration.
  ...
  ImGuiInputTextFlags_NoMarkEdited = 1 << 21  // For internal use by functions
                                              // using InputText() before
                                              // reformatting data
};
...
enum ImGuiTabBarFlags_ {
  ...
  ImGuiTabBarFlags_Reorderable = 1 << 0,  // Allow manually dragging tabs to
                                          // re-order them + New tabs are
...
enum ImGuiTabItemFlags_ {
  ...
  ImGuiTabItemFlags_SetSelected = 1 << 1,  // Trigger flag to programmatically
                                           // make the tab selected when calling
                                           // BeginTabItem()
...
// Note: windows with the ImGuiWindowFlags_NoInputs flag are ignored by
// IsWindowHovered() calls.
enum ImGuiHoveredFlags_ {
  ImGuiHoveredFlags_None = 0,  // Return true if directly over the item/window,
                               // not obstructed by another window, not
                               // obstructed by an active popup or modal
...
// Back-end: set ImGuiBackendFlags_HasGamepad and fill the io.NavInputs[] fields
// before calling NewFrame(). Note that io.NavInputs[] is cleared by EndFrame().
// Read instructions in imgui.cpp for more details. Download PNG/PSD at
// http://goo.gl/9LgVZW.
enum ImGuiNavInput_ {
  // Gamepad Mapping
...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_NoMouse = 1 << 4,  // Instruct imgui to clear mouse
                                      // position/buttons in NewFrame(). This
                                      // allows ignoring the mouse information
                                      // set by the back-end.
...
enum ImGuiColorEditFlags_ {
  ...
  ImGuiColorEditFlags_Uint8 = 1 << 23,       // [DataType]   // ColorEdit,
  ...
  ImGuiColorEditFlags_PickerHueBar = 1 << 25,    // [Picker]     // ColorPicker:
                                                 // bar for Hue, rectangle for
                                                 // Sat/Value.
  ImGuiColorEditFlags_PickerHueWheel = 1 << 26,  // [Picker]     // ColorPicker:
                                                 // wheel for Hue, triangle for
                                                 // Sat/Value.
  ImGuiColorEditFlags_InputRGB = 1 << 27,        // [Input]      // ColorEdit,
                                           // ColorPicker: input and output data
                                           // in RGB format.
  ImGuiColorEditFlags_InputHSV = 1 << 28,  // [Input]      // ColorEdit,
                                           // ColorPicker: input and output data
                                           // in HSV format.
  ...
  ImGuiColorEditFlags__OptionsDefault = ImGuiColorEditFlags_Uint8 |
                                        ImGuiColorEditFlags_DisplayRGB |
...
// Important: Treat as a regular enum! Do NOT combine multiple values using
// binary operators! All the functions above treat 0 as a shortcut to
// ImGuiCond_Always.
enum ImGuiCond_ {
  ImGuiCond_Always = 1 << 0,  // Set the variable
  ImGuiCond_Once = 1 << 1,    // Set the variable once per runtime session (only
                              // the first call with succeed)
  ImGuiCond_FirstUseEver = 1 << 2,  // Set the variable if the object/window has
                                    // no persistently saved data (no entry in
                                    // .ini file)
  ImGuiCond_Appearing = 1 << 3      // Set the variable if the object/window is
                                // appearing after being hidden/inactive (or the
                                // first time)
};
...
// before NewFrame().
// During the frame, use ImGui::PushStyleVar(ImGuiStyleVar_XXXX)/PopStyleVar()
...
struct ImGuiIO {
  ...
  ImGuiBackendFlags BackendFlags;  // = 0              // See ImGuiBackendFlags_
                                   // enum. Set by back-end (imgui_impl_xxx
                                   // files or custom back-end) to communicate
  ...
  // Render(). Please call your rendering function yourself now!
  // You can obtain the ImDrawData* by calling ImGui::GetDrawData() after
  // Render(). See example applications if you are unsure of how to implement
  // this.
  ...
  ImVector<ImWchar> InputQueueCharacters;  // Queue of _characters_ input
                                           // (obtained by platform back-end).
                                           // Fill using AddInputCharacter()
                                           // helper.
...
// Callbacks (follow a flag name and see comments in ImGuiInputTextFlags_
...
typedef ImGuiInputTextCallback ImGuiTextEditCallback;  // OBSOLETED in 1.63
                                                       // (from Aug 2018): made
...
// every frame.
// Usage: static ImGuiOnceUponAFrame oaf; if (oaf) ImGui::Text("This will be
...
// This is optimized for efficient lookup (dichotomy into a contiguous buffer)
// and rare insertion (typically tied to user interactions aka max once a frame)
...
// widget or customized UI rendering,
// you can poke into the draw list for that! Draw callback may be useful for
// example to:
...
// The renderer back-end needs to handle this special value, otherwise it will
// crash trying to call a function at this address.
// This is useful for example if you submitted callbacks which you know have
// altered the render state and you want it to be restored.
// It is not done by default because they are many perfectly useful way of
// altering render state for imgui contents (e.g. changing shader/blending
...
// is enabled, those fields allow us to render meshes larger than 64K vertices
...
// declared a the time you'd want to set your type up.
...
// which can be drawn into out of order.
// This is used by the Columns api, so items of each column can be batched
...
enum ImDrawListFlags_ {
  ...
  ImDrawListFlags_AntiAliasedFill = 1 << 1,  // Filled shapes have anti-aliased
                                             // edges (*2 the number of
                                             // vertices)
...
// all command lists are passed to your ImGuiIO::RenderDrawListFn function for
// rendering.
// Each dear imgui window contains its own ImDrawList. You can use
// ImGui::GetWindowDrawList() to
...
// bottom-right at io.DisplaySize), but you are totally free to apply whatever
// transformation matrix to want to the data (if you apply such transformation
// you'll want to apply it to ClipRect as well)
...
struct ImDrawList {
  ...
  unsigned int _VtxCurrentOffset;  // [Internal] Always 0 unless 'Flags &
                                   // ImDrawListFlags_AllowVtxOffset'.
  unsigned int _VtxCurrentIdx;  // [Internal] Generally == VtxBuffer.Size unless
  ...
  //   Use AddNgon() and AddNgonFilled() functions if you need to guaranteed a
  ...
  IMGUI_API void AddRectFilled(const ImVec2& p_min,
                               ...
                               ImDrawCornerFlags rounding_corners =
                                   ImDrawCornerFlags_All);  // a: upper-left, b:
                                                            // lower-right (==
                                                            // upper-left +
                                                            // size)
  ...
  IMGUI_API void AddConvexPolyFilled(const ImVec2* points,
                                     int num_points,
                                     ImU32 col);  // Note: Anti-aliased filling
                                                  // requires points to be in
                                                  // clockwise order.
  IMGUI_API void AddBezierCurve(const ImVec2& p1,
                                const ImVec2& p2,
                                const ImVec2& p3,
                                const ImVec2& p4,
                                ImU32 col,
  ...
  IMGUI_API void PathBezierCurveTo(const ImVec2& p2,
                                   const ImVec2& p3,
                                   const ImVec2& p4,
                                   int num_segments = 0);
  ...
  // place!
  //   Prefer using your own persistent copy of ImDrawListSplitter as you can
  //   stack them.
...
// currently preserve them for backward compatibility purpose,
// as this is one of the oldest structure exposed by the library! Basically,
// ImDrawList == CmdList)
struct ImDrawData {
  ...
  ImDrawList** CmdLists;  // Array of ImDrawList* to render. The ImDrawList are
                          // owned by ImGuiContext and only pointed to from
                          // here.
...
struct ImFontAtlasCustomRect {
  unsigned int ID;  // Input    // User ID. Use < 0x110000 to map into a font
                    // glyph, >= 0x110000 for other/internal/custom texture
                    // data.
...
struct ImFontAtlas {
  ...
  IMGUI_API void ClearTexData();  // Clear output texture data (CPU side). Saves
                                  // RAM once the texture has been copied to
                                  // graphics memory.
  IMGUI_API void
  ...
  // NB: Consider using ImFontGlyphRangesBuilder to build glyph ranges from
  // textual data.
  ...
  IMGUI_API const ImWchar* GetGlyphRangesJapanese();  // Default + Hiragana,
                                                      ...
                                                      // Selection of 1946
                                                      // Ideographs
  IMGUI_API const ImWchar* GetGlyphRangesChineseFull();  // Default + Half-Width
                                                         // + Japanese
                                                         // Hiragana/Katakana +
                                                         // full set of about
                                                         // 21000 CJK Unified
  ...
  IMGUI_API int AddCustomRectRegular(unsigned int id,
                                     int width,
                                     int height);  // Id needs to be >=
                                                   // 0x110000. Id >= 0x80000000
                                                   // are reserved for ImGui and
                                                   // ImDrawList
  IMGUI_API int AddCustomRectFontGlyph(
      ...
      int height,
      float advance_x,
      const ImVec2& offset = ImVec2(0, 0));  // Id needs to be < 0x110000 to
...
struct ImFont {
  ...
  ImVector<float> IndexAdvanceX;  // 12-16 // out //            // Sparse.
                                  // Glyphs->AdvanceX in a directly indexable
                                  // way (cache-friendly for CalcTextSize
                                  // functions which only this this info, and
  ...
  short ConfigDataCount;           // 2     // in  // ~ 1        // Number of
                          // ImFontConfig involved in creating this font. Bigger
                          // than 1 when merging multiple font sources into one
                          // ImFont.
  ImWchar FallbackChar;  // 2     // in  // = '?'      // Replacement character
                         // if a glyph isn't found. Only set via
                         // SetFallbackChar()
